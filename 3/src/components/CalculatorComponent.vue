...
function operationToJS(tokens, index) {
  let nextValue = () =>
    tokensToJS([getOrThrow(tokens, index + 1, "Argument must be provided")]);
  switch (tokens[index].value) {
    case BUTTONS.SIN.value:
      return [1, `Math.sin(${nextValue()})`];
    case BUTTONS.COS:
      return [1, `Math.cos(${nextValue()})`];
    case BUTTONS.LOG:
      return [1, `Math.log(${nextValue()})`];
    case BUTTONS.TAN.value:
      return [1, `Math.tan(${nextValue()})`];
    case BUTTONS.OPEN.value:
      return [0, `(`];
    case BUTTONS.CLOSE.value:
      return [0, `)`];
    case BUTTONS.MULT.value:
      return [0, "*"];
...
@media only screen and (min-width: 600px) {
  body {
    display: grid;
    grid-template-columns: calc(50vw - (var(--calc-width)) * 0.5) 3fr calc(
        50vw - (var(--calc-width)) * 0.5
      );
  }
}*/
</style>